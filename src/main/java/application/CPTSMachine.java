...
public class CPTSMachine {
	...
	public static boolean addStation(String name) {
        ...
        if(!isStationExisted(name)) {
            setTotalStationNumber(totalStationNumber+1);
	...
	public static boolean buyTicket(int type, Station start, Station end) {
        ...
        if(type >=0 && type <=2 && price != -1) {
			/* FILL CODE */
			System.out.println("Bought "+T.getDescription()+", for "+price+" Baht!");
			return true;
		}else {
    ...
    public static void setTotalStationNumber(int totalStationNumber) {
        CPTSMachine.totalStationNumber = totalStationNumber;
    }
}
